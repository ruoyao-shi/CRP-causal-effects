...
outcome_names <- c(
  `SCZ` = "Schizophrenia (-)",
  `T2D` = "Type 2 Diabetes (+,x)",
  `stroke` = "Stroke (+,-,x)",
  `Alzheimer` = "Alzheimer's Disease (x)",
  `Knee` = "Knee Osteoarthritis (+)",
  `Parkinson` = "Parkinson's Disease (-,x)",
  `Colorectal` = "Colorectal Cancer (-,x)",
  `IBD` = "Inflammatory Bowel Disease (-)",
  `BD` = "Bipolar Disorder (+,x)",
  `BMD` = "Bone Mineral Density (x)",
  `RA` = "Rheumatoid Arthritis (+,x)",
  `CAD` = "Coronary Artery Disease (x)",
  `HDL` = "HDL Cholesterol (-)",
  `LDL` = "LDL Cholesterol (-)",
  `Triglycerides` = "Triglycerides (-)",
  `HbA1c` = "Glycated Hemoglobin (+)")
for (i in 1:length(label)) {
  res_0.05 <- read.csv(paste0(id_list[i],"/results/",label[i],"_MVMR_GRAPPLE_FDR_p_0.05_summary_new.csv"))
  res_0.1 <- read.csv(paste0(id_list[i],"/results/",label[i],"_MVMR_GRAPPLE_FDR_p_0.1_summary_new.csv"))
  res_MR <- read.csv(paste0(id_list[i],"/results/",label[i],"_MR_GRAPPLE_FDR_p_0.05_summary_new.csv"))
...
p2 <- all_res_0.1 %>% 
  ...
  xlab("Odds Ratio or Linear Association (95% CI)") + coord_flip() +
...
#ggsave("all_summary_0.1_new.png", plot = p2,width = 15, height = 15)
...
p3 <- all_res_MR %>% 
  ...
  theme(axis.text.y = element_text(size = 15),
        ...
        axis.title.y = element_text(size = 15),
        legend.position = "bottom")
#ggsave("all_summary_MR_new.png", plot = p3,width = 15, height = 15)
...
p5 <- all_res_0.05 %>%
  ...
  geom_text(aes(y = type,
                ...
                label = sig,
                group = type),
            position = position_dodge(width = 0.9), vjust = -0.01, color = "black",size = 6) +
  xlab("Odds Ratio or Linear Association (95% CI)") + ylab("Analysis Type") + coord_flip() +
  facet_wrap(~factor(outcome,levels = c('stroke','CAD','HDL','LDL','Triglycerides',
                                        'T2D','HbA1c','IBD','RA','SCZ','BD',
                                        'Alzheimer','Parkinson','Colorectal',
                                        'Knee','BMD')), ncol=4, scales = "free_y",
             labeller = as_labeller(outcome_names))+
  theme_bw() +
  scale_color_manual(values = c("UVMR" = "#D81B60", "only BMI" = "#1E88E5", "MVMR" = "#E69F00")) + 
...
ggsave("Fig 3_new.png", plot = p5, dpi = 300, width = 22, height = 20)
p6 <- all_res_0.05 %>%
  mutate(xintercept = if_else(outcome %in% continuous_outcomes, 0, 1),
         sig = case_when(
           pvalue < 0.05/16 ~ "**",
  ...
  theme(axis.text.y = element_text(size = 20),
        ...
        axis.title.x = element_text(size = 23),
        axis.title.y = element_text(size = 23))
ggsave("Fig S1_new.png", plot = p6, dpi = 300, width = 22, height = 20)
p7 <- all_res_0.05 %>%
  mutate(xintercept = if_else(outcome %in% continuous_outcomes, 0, 1),
         sig = case_when(
           pvalue < 0.05/16 ~ "**",
  ...
  theme(axis.text.y = element_text(size = 20),
        ...
        axis.title.x = element_text(size = 23),
        axis.title.y = element_text(size = 23))
ggsave("Fig S2_new.png", plot = p7, dpi = 300, width = 22, height = 20)
p8 <- all_res_0.05 %>%
  mutate(xintercept = if_else(outcome %in% continuous_outcomes, 0, 1),
         sig = case_when(
           pvalue < 0.05/16 ~ "**",
  ...
  theme(axis.text.y = element_text(size = 20),
        ...
        axis.title.x = element_text(size = 23),
        axis.title.y = element_text(size = 23))
ggsave("Fig S3_new.png", plot = p8, dpi = 300, width = 22, height = 20)
